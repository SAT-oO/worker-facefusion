...
TRIM_FRAME_RANGE_SLIDER : Optional[gradio.Slider] = None
...
def render() -> None:
	...
	TRIM_FRAME_RANGE_SLIDER = gradio.Slider(**trim_frame_range_slider_options)
...
def remote_update() -> gradio.Slider:
	if is_video(state_manager.get_item('target_path')):
		video_frame_total = count_video_frame_total(state_manager.get_item('target_path'))
		state_manager.clear_item('trim_frame_start')
		state_manager.clear_item('trim_frame_end')
		return gradio.Slider(value = (0, video_frame_total), maximum = video_frame_total, visible = True)
