...
画质/延迟调优可参考 [quality_tiers.json](quality_tiers.json)（将某一档的 `extra_args` 与 `face_swapper_model` 写入 `input`）。
...
| [quality_tiers.json](quality_tiers.json)                           | 画质档位预设（按算力耗时递增的 `extra_args`，共 5 档）        |
