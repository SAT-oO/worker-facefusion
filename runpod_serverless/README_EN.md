...
**Quality tiers** — [`quality_tiers.json`](quality_tiers.json) lists **5 presets** ordered by ascending compute time. Tier 1 (`production`) is the full source-matched video baseline; higher tiers add face detail or encode fidelity.

Benchmark profiles in `[runpod_serverless/tests/benchmark/config.json](runpod_serverless/tests/benchmark/config.json)` (overlap with tiers 1, 2, and 4):


| Profile               | Intent                                                                                      |
| --------------------- | ------------------------------------------------------------------------------------------- |
| `production`          | Full source-matched baseline: 128×128 boost, NVENC medium (tier 1).                         |
| `fast_nvenc`          | Faster NVENC encode variant: 256×256 boost, 4 threads (tier 2).                             |
| `fast_nvenc_threads8` | Same with 8 execution threads.                                                              |
| `baseline_e2e`        | Quality-first: `libx264`, `veryslow`, 512×512 boost (tier 4).                               |
...
| `BENCHMARK_TARGET_60S_URL`  | Benchmark only | R2 URL for 60 s fixture (optional; defaults in `benchmark_common.py`). |
...
# Edit endpoint_id in config.json if not using ENDPOINT_ID env
...
| `[quality_tiers.json](quality_tiers.json)`           | Incremental quality/latency presets (`extra_args` by rank) |
| `[submit_job.py](submit_job.py)`                     | CLI to POST jobs and optional poll |
| `[Dockerfile](../Dockerfile)`                        | Production image (repo root)      |
