...
基准配置档见 [tests/benchmark/config.json](tests/benchmark/config.json)（目标视频 URL 默认由 `benchmark_common.py` 提供，可通过 `BENCHMARK_TARGET_*_URL` 或 `--target-url` 覆盖）：


| 配置档（Profile）          | 定位                                                              |
| --------------------- | --------------------------------------------------------------- |
| `baseline_e2e`        | 画质优先：`libx264`、`veryslow`、512×512 boost，输出与源片同分辨率/帧率       |
| `fast_nvenc`          | 延迟目标：CUDA、NVENC、256×256 boost、4 线程，输出与源片同分辨率/帧率          |
| `fast_nvenc_threads8` | 同上，8 执行线程                                                       |
| `production`          | 生产基线：128×128 boost、NVENC medium，输出与源片同分辨率/帧率（tier 1）       |
